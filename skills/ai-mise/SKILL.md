...
The control plane is not inside this skill. Look for it at
`~/ai-mise/control-plane/`, where `INSTALL.md` puts it, or in the
repository if you are running from a clone. Where neither has it the
install did not finish — say so, and point at `INSTALL.md`. Do not
