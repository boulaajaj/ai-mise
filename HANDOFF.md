...
1. **Baselines** (needs Amine's machine): run plain `/init` (try `CLAUDE_CODE_NEW_INIT=1`) on the same materials the bootstrapper is given, and snapshot the output. Store under `control-plane/evaluation/baselines/` (or record paths if too large). A baseline exists to compare like with like (ADR-0011).
