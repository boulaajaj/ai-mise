...
- **Phase 1 — Read-only bootstrapper**: one first-contact flow covering both cases, source inventory where there are materials, unknowns ledger, decision-aware questions, workspace proposal. No writes, no wiki, no SQLite, no self-improvement. Exit (ADR-0011): from nothing — no folder, no domain — it reaches a proposal you would seriously consider approving, showing no lean toward any profession. Run against existing materials instead, first contact produces a proposal that cites them.
