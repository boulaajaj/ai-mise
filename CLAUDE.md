...
- Read every finding in a review in full before answering any of them. A summary count and a truncated preview are not findings: a review announcing six actionable comments has six, whatever a notification shows of them. Fetch the comment bodies, verify each against the code it names, and answer all of them in one disposition. (2026-08-24: round one on #132 answered two of six, having worked from the preview list rather than the comments.)
...
- Rounds one and two fix what review found. A third round means the change is not saying what it means. **At a fourth round, stop and do not patch again.** A finding that survives three fixes is evidence that the instruction, the design, or these rules are wrong — fix that instead, and say in the PR which of the three it was. Hacking a change until review goes quiet is the failure this rule exists to prevent. Count recurrence, not rounds: distinct findings that shrink in scope and severity each round are a review converging, and stopping there ships a defect you have already been shown. What says the change is wrong is the same finding returning after a fix, or a new one of the same kind. (2026-08-24: #132 ran to five rounds - six Major findings, then two, then one, then one Minor, each of them different - and counting rounds would have shipped a known bug in an evaluation instrument in order to honour a rule about not hacking.)
